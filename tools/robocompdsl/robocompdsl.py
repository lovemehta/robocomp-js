...
if sys.argv[1].endswith(".cdsl"):
	...
	if component['language'].lower() == 'cpp':
		...
		# Generate regular files
		#
		files = ['src/component.js','src/UI/UI.html']
		specificFiles = ['src/UI/UI.html', 'src/component.js']
		for f in files:
			ofile = outputPath + '/' + f
			if f in specificFiles and os.path.exists(ofile):
				print 'Not overwriting specific file "'+ ofile +'", saving it to '+ofile+'.new'
		...
		for ima in component['implements']:
			...
			if communicationIsIce(ima):
				for f in [ "SERVANT.JS"]:
					...
					print 'Generating ', ofile, ' (servant for', im + ')'
					# Call cog
					run = "cog.py -z -d -D theCDSL="+inputFile  + " -D theIDSLs="+imports + ' -D theIDSLPaths='+ '#'.join(includeDirectories) + " -D theInterface="+im + " -o " + ofile + " " + "/opt/robocomp/share/robocompdsl/templateJS/" + f
					run = run.split(' ')
					ret = Cog().main(run)
					if ret != 0:
						print 'ERROR'
